...
class CA_LSTM(nn.Module):
	...
	def __init__(self, in_dim, out_dim, sent_len, layers=1, batch_first=True, bidirectional=True, dropoutP=0.3):
		...
		self.map_linear = nn.Linear(3*in_dim+sent_len, 4*in_dim)
	...
	def forward(self, p_input, q_input, o_input):
		...
		# (bsz*no_opt*no_sent, sent_len, 3*in_dim+sent_len)
...
class HCAN(nn.Module):
	def __init__(self, args, num_choices=3):
		...
		self.sent_lstm = CA_LSTM(args.d_model, args.d_lstm, args.max_sent_len, layers=args.lstm_layers)
		self.para_lstm = CA_LSTM(2*args.d_lstm, 2*args.d_lstm, args.max_sent_len, layers=args.lstm_layers)
	...
	def forward(self, passage_input, question_input, option_input, labels):
		"""
		...
		p_emb_outputs = self.sent_xlnet(p_input_ids.contiguous().view(-1, sent_len), token_type_ids=p_token_type_ids.contiguous().view(-1, sent_len), attention_mask=p_attn_mask.contiguous().view(-1, sent_len))
		...
		p_emb = p_emb.repeat(self.no_opt, 1, 1, 1) #(bsz*no_opt, no_sent, sent_len, hid_dim)
		...
		q_emb = q_emb.repeat(self.no_opt, 1, 1) #(bsz*no_opt, sent_len, hid_dim)
		...
		sent_attn = F.softmax(sent_attn, dim=-1) #(bsz*no_opt*no_sent, sent_len)
		...
		para_attn = F.softmax(para_attn, dim=-1) #(bsz*no_opt, no_sent)
		...
		para_vec = para_vec.view(-1, self.no_opt, para_vec.size(-1)) #(bsz, no_opt, 4*d_lstm)
